...
import { SettingsHeader } from './SettingsRow';
import { useLanguage } from '../context/LanguageContext';

export function LegalPagePending({ title }: { title: string }) {
  const { t } = useLanguage();
  return (
    <SafeAreaView className="flex-1 bg-[#F4F5F8]" edges={['top']}>
      <SettingsHeader title={title} onBack={() => router.back()} />

/**
 * The sign-in form, shared by the full-page route and the sign-in sheet (D-151).
 *
 * On the web a reader can browse without an account and is asked to sign in
 * only when they try to do something that needs one. That ask is a sheet over
 * the page they are on, not a navigation away from it — so the form has to
 * live somewhere both can render. It was lifted out of app/(auth)/login.tsx
 * unchanged; the page is now this form inside its Screen and header.
 */
...
export function LoginForm({
  ...
  /** `modal` drops the status bar and closes itself before navigating away. */
  variant?: 'page' | 'modal';
  /** Called before the form navigates to register / forgot password. */
  onLeave?: () => void;
}) {
  const { t } = useLanguage();
  ...
  return (
    <>
      {variant === 'page' ? <StatusBar style="light" /> : null}
        {/* Welcome heading */}
        <Text
          style={{
        ...
        >
          ...
          <Pressable
            ...
            onPress={() => {
              onLeave?.();
              if (variant === 'modal') router.push('/register');
